...
# divided by the total number of requested confirmation messages. The confirmation rate
...
WITH action_stat AS (
    SELECT
        ...
        SUM(CASE WHEN action='confirmed' THEN 1 END) AS "action_confirmed"
    FROM
        Confirmations
    GROUP BY
        user_id
...
SELECT s.user_id,
       COALESCE(ROUND(action_confirmed::numeric / action_total, 2), 0) AS "confirmation_rate"
FROM Signups s
         LEFT JOIN
     action_stat a ON a.user_id = s.user_id # There is no code of Python type for this problem